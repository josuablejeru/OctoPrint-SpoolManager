...
class SpoolmanagerPlugin(
    ...
    def checkRemainingFilament(self, forToolIndex=None):
        ...
        :return: see
        """
        shouldWarn = self._settings.get_boolean(
    ...
    def set_temp_offsets(self, toolIndex, spoolModel):
        ...
        if toolOffsetEnabled == True and spoolModel != None:
            ...
            offset_dict["tool" + str(toolIndex)] = (
                spoolModel.offsetTemperature
        ...
        if bedOffsetEnabled == True and spoolModel != None:
            if spoolModel.offsetBedTemperature != None:
                if (
                    self._isNewOffsetTemperatureGreater(
                        "bed", spoolModel.offsetBedTemperature
                    )
                    == True
                ):
        ...
        if enclosureOffsetEnabled == True and spoolModel != None:
            if spoolModel.offsetEnclosureTemperature != None:
                if (
                    self._isNewOffsetTemperatureGreater(
                        "chamber", spoolModel.offsetEnclosureTemperature
                    )
                    == True
                ):
    ...
    def _isNewOffsetTemperatureGreater(self, selectedOffset, newOffset):
        ...
        selectedTemperature = (
            allTemperatures[selectedOffset]
            if selectedOffset in allTemperatures
            else None
        )
                ...
                if origin != None and path != None:
                    ...
                    if "analysis" in metadata:
                        if "filament" in metadata["analysis"]:
                            for toolName, toolData in metadata["analysis"][
                                "filament"
                            ].items():
                                ...
                                self.metaDataFilamentLengths[toolIndex] = toolData[
                                    "length"
                                ]
            ...
            if selectedSpool != None:
                ...
                if missing_fields:
                    ...
                    if not_a_number_fields:
                        ...
                        saftyLengthInMM = self._settings.get_int(
                            [SettingsKeys.SETTINGS_KEY_SAFETY_LENGTH]
                        )
                        if saftyLengthInMM != 0:
                            saftyRequiredWeight = self._calculateWeight(
                                saftyLengthInMM, diameter, density
                            )
                            self._logger.info(
                                "saftyWeight '"
                                + str(saftyRequiredWeight)
                                + "' from saftyLengthInMM '"
                                + str(saftyLengthInMM)
                                + "' calculated"
                            )
    ...
    def commitOdometerData(self):
        ...
        for toolIndex, spoolModel in enumerate(selectedSpools):
            ...
            spoolUsedLength = (
                0.0
                if StringUtils.isEmpty(spoolModel.usedLength) == True
            ...
            if diameter is None or density is None:
                ...
                spoolUsedWeight = (
                    0.0 if spoolModel.usedWeight == None else spoolModel.usedWeight
                )
    ...
    def get_settings_defaults(self):
        ...
        ## Export / Import
        settings[
            SettingsKeys.SETTINGS_KEY_IMPORT_CSV_MODE
        ...
        datbaseLocation = DatabaseManager.buildDefaultDatabaseFileLocation(
